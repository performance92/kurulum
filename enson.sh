...
# Eski ana sunucuyu yedekten senkronize et ve standby moduna al
sync_primary_with_standby_and_promote() {
  ...
  if [ $? -eq 0 ]; then
    ...
    sleep 20
    echo "Ana sunucu tekrar primary olarak terfi ettirildi. Yedek sunucuyu standby moduna alıyoruz..."
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "systemctl stop postgresql@14-main.service || systemctl stop postgresql.service"
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "rm -rf $DATA_DIR/*"
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "PGPASSWORD=$STANDBY_PASSWORD pg_basebackup -h $PRIMARY_IP -D $DATA_DIR -U $STANDBY_USER -P -v -R -X stream -C -S slaveslot3"
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "sudo chown -R postgres:postgres /var/lib/postgresql/14/main"
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "sudo chmod 700 /var/lib/postgresql/14/main"
    sshpass -p "$SSH_PASSWORD" ssh root@$STANDBY_IP "systemctl start postgresql@14-main.service || systemctl start postgresql.service"
    echo "Yedek sunucu standby moduna alındı ve ana sunucu ile senkronize durumda."
  else
